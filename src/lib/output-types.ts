export const OUTPUT_TYPES = [
  ...
  { id: "meeting_notes", label: "Meeting Minutes", icon: "📋", system: "Convert the transcript into structured meeting minutes with sections: Attendees (if mentioned), Agenda, Discussion, Decisions, and Action Items (with owners and due dates when mentioned). Use plain text with section labels and bullet lists — no markdown symbols." },
  { id: "tasks", label: "Task List", icon: "✅", system: "Extract every actionable item from the transcript and format as a checklist using \"- \" bullets. Group by owner if multiple people are mentioned. Be specific and outcome-focused. No markdown heading symbols." },
